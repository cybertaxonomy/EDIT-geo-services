...
--2) Table test_"csvimportgispoints" (table containing the geometry field, referencing "test_csvimportpk")

 CREATE TABLE test_csvimportgispoints
 (
   pk serial NOT NULL,
   fk_toimportpk integer,
   geomfield geometry,
   CONSTRAINT test_csvimportgispoints_pk PRIMARY KEY (pk),
   CONSTRAINT test_csvimportgispoints_to_test_csvimport_fk FOREIGN KEY (fk_toimportpk)
      REFERENCES test_csvimportpk (pk) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
 ) 
 WITH OIDS;
 ALTER TABLE test_csvimportgispoints OWNER TO postgres;
...
--4) Script of the pl/pgsql function
 ...
 CREATE OR REPLACE FUNCTION edit_import_csvgis(filename character varying, nbcolumns   integer, positionx integer, positiony integer, datumid integer, csvdelimiter character varying, usercode character varying, currentdate timestamp without time zone)
 RETURNS integer AS
 $BODY$
 /*author first version: franck theeten 16/09/2007*/
 DECLARE
 ...
 --tmpcolsinsert:=tmpcolsinsert||', field'||currentColStr;
 ----value for user code
 --tmpvalsinsert:=tmpvalsinsert||', <nowiki>'''||usercode||''''</nowiki>;   
 
 --terminate SQL statement for creation of temporary table
 createtabstr:= createtabstr||');';
 --SQL statement for copying the CSV;
 copytabstr:='COPY tmp_import FROM <nowiki>E'''||filename||'''</nowiki> WITH CSV HEADER '; 
 --IF csvdelimiter!=',' THEN
  copytabstr:=copytabstr||' DELIMITER AS<nowiki> '''||csvdelimiter||''''</nowiki>;
 --END IF;
 ...
 inserttabstr:='INSERT INTO test_csvimportpk (pk, '||tmpcolsinsert||', userid, importationdate) SELECT nextval(''test_csvimportpk_pk_seq''), '||tmpvalsinsert||', <nowiki>'''||usercode::varchar||'''</nowiki>, '''||currentdate||''' FROM tmp_import;';
 insertGIStabstr:='INSERT INTO test_csvimportgispoints (pk, fk_toimportpk, geomfield) SELECT nextval(''test_csvimportgispoints_pk_seq''), pk, POINTFROMTEXT(''POINT(''||'||xgiscol||'||'' ''||'||ygiscol||'||'')'','||datumid::varchar||' ) FROM test_csvimportpk;';
 ...
 ALTER FUNCTION edit_import_csvgis(filename character varying, nbcolumns integer,  positionx integer, positiony integer, datumid integer, csvdelimiter character varying, usercode character varying, currentdate timestamp without time zone) OWNER TO postgres;
