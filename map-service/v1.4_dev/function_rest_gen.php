...
//print($p_externalwms_filter);
				...
				//ob_start();
				$flagGoodHeaders=TRUE;
				$conn = pg_pconnect(POSTGIS_CS);
				$flagError=false;
				$flagIndexerTDWG=false;
				$indexer_field="";
				$errorMessages=array();
				...
				//$path_towrite_persistent_sld="/var/www/synthesys/www/v1.4/sld_persistent/";
				//$url_persistent_sld='http://edit.br.fgov.be/synthesys/www/v1.4/sld_persistent';
				...
				$ratio_y=NULL;
				$leg=$p_legend;
				$flagReprojection=false;
				$bboxReprojection="";
				$sld_url_displayed_legend=NULL;
				$sldFileNameAfterLegendRewriting=NULL;
				$alreadyProcessedLegends=Array();
				
				if(isset($p_dest_epsg_projection))
				{
					if(strlen($p_dest_epsg_projection)>0)
					{
						$flagReprojection=true;
					}				
				}
				...
				$sld_dir = "/var/www/synthesys/www/v1.4/sld"; 
				
				$img_dir = "/var/www/synthesys/www/v1.4/img";
				...
				
				{
					...
					print("Eg. http://edit.br.fgov.be/edit_wp5/v1.4/areas.php<b>?</b>*Layers<b>&</b>Area Data<b>&</b>Area Style<b>&</b>*Bounding Box<b>&</b>Map Size<br>");
				...
				if($flagDisplayPoints===true)
				{
					...
					$path_towrite="/var/www/synthesys/www/v1.4/sld/point_".$random.".sld";
							...
							{//for v
								...
								{//if k
									...
									*/
									...
									{//loop first pol
										...
										$tdwg[$first_layer][$first_style]['field']=$first_wms_field;
										//ftheeten 2010/02/21
								...
								{//if k	
									...
									{//if data 1
										...
										$tdwg[$first_layer][$data[0]]['field']=$first_wms_field;
				...
				$leg_path_towrite="/var/www/synthesys/www/v1.4/sld/$random";
				
				$fp=fopen("$leg_path_towrite","w+");
				
				$write=fwrite($fp,$out);
				
				//$leg_url="/var/www/synthesys/www/v1.4/sld/$random";
				...
				{//loop tdwg 1
					...
					{// loop style=>value
						...
						{//loop key=> country
							...
							if(!is_array($country)&&$key=="zones")//ftheeten 2013/09/11
							...
							elseif(!is_array($country)&&$key=="field")
							{
								
								if(isset($country))//cause weird problem with tdwg layer sld 2013/09/26
								{
									if(strlen($country)>0)
									{
										$xml.="<indexerfield>".$country."</indexerfield>";
									}
								}
								$indexer_field=$country;
								$flagIndexerTDWG=TRUE;
							}
				...
				
							{
								...
								{
									//print_r($tdwg[$k]);									
									//$xml.="<indexerfield>".$country[0]."</indexerfield>";
								}
								$idxField=0;
								foreach ($tdwg[$k][$style]['zones'] as $zones)
				
								{
									$xml.="<indexerfield>".$tdwg[$k][$style]['field'][$idxField]."</indexerfield>";
...
//print($bbox);
				...
				
				{
					...
					$json=$p_callback.'({"bbox":"'.$bbox.'","layers":[';
...
//print("GML=");				
				...
				foreach ($gmls as $feature=>$v)
				{
					...
					switch ($feature)
					{
						...
						default:
							...
							else
							{
								...
								//print($feature);
								//print("WMS_FIELD_ARRAY");
								//print_r($wms_field_array);
								//print("field=");
								//print($field);
								//print("STYLE_BIS=");
								//print($tdwg[0][0]['field']);
					...
					//ftheeten 2013/09/26
					if($flagIndexerTDWG===TRUE)
					{
						
						//$tmpPathStyleName=new DOMXPath($dom_new);
						//$dom_styles=$tmpPathStyleName->query("//gml/style");
						$dom_styles=$dom_new->getElementsByTagName("style");					
											
						for($j=0;$j<$dom_styles->length;$j++)
						{	
							$dom_st=$dom_styles->item($j);	
							//print("STYLEISFOUND");
							//print($dom_st->textContent);
							$recurs_dom=new DomDocument();
							$recurs_dom->loadXML("<root></root>");
							$recurs_dom->documentElement->appendChild($recurs_dom->importNode($dom_st,true));
							
							$tmpPathStyleName_recurs=new DOMXPath($recurs_dom);
							$dom_indexers=$tmpPathStyleName_recurs->query("//root/style/indexerfield");
													
							if($dom_indexers->length==0&&strlen($indexer_field)>0)						
							{
								
								//$tmpDoc=new DomDocument();
								//$tmpDoc->loadXML("<indexerfield>".$indexer_field."</indexerfield>");
								$tmpDoc=new DomElement("indexerfield", $indexer_field);
								$dom_st->appendChild($tmpDoc);
							}
					...
					$path_towrite="/var/www/synthesys/www/v1.4/sld/".$feature."_".$random;
					//print($path_towrite);
					//print($out);
					//echo $path_towrite;
					...
					if($flagReprojection===true)
					{
						...
						$bbox_dest_proj_tmp=explode(",",$bbox);
	
						$point1_conv=convertPointProjection($conn,$bbox_dest_proj_tmp[1],$bbox_dest_proj_tmp[0],"4236",$p_dest_epsg_projection);
						$point2_conv=convertPointProjection($conn,$bbox_dest_proj_tmp[3],$bbox_dest_proj_tmp[2],"4236",$p_dest_epsg_projection);
						$bboxReprojection=$point1_conv.",".$point2_conv;
					...
					if(strlen($layer)>0||((strlen($urlExternalWMS)>0)&&$flagExternalWMS===TRUE))
					{
						...
						//ftheeten 2014/06/25
						$sldBeforeLegendRewriting=file_get_contents(V1_SLD_URL."/".$nameSLDFile);
						$xmlBodySLDLegend.=writeSLDForLegend_areas($sldBeforeLegendRewriting, $p_title,$alreadyProcessedLegends);
						//print($url);		
						$url_list[]=$url;
				...
				//ftheeten 2014/06/25 (specific SLD for legend)
				$xmlSLDBegin="<StyledLayerDescriptor xmlns=\"http://www.opengis.net/sld\" xmlns:ogc=\"http://www.opengis.net/ogc\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:wfs=\"http://www.opengis.net/wfs\" version=\"1.0.0\" xsi:schemaLocation=\"http://www.opengis.net/sld StyledLayerDescriptor.xsd3\"><NamedLayer><Name>topp:cyprusdivs</Name><UserStyle><FeatureTypeStyle>";
				$xmlSLDEnd="</FeatureTypeStyle></UserStyle></NamedLayer></StyledLayerDescriptor>";
				$sldFileNameAfterLegendRewriting="sld_displayed_legend_".$nameSLDFile;
				$path_sldFileNameAfterLegendRewriting=V1_SLD_PATH."/".$sldFileNameAfterLegendRewriting;
				$fp=fopen($path_sldFileNameAfterLegendRewriting,"w+");
				if($fp===FALSE)
				{
				...
				$out=$xmlSLDBegin.$xmlBodySLDLegend.$xmlSLDEnd;
				
				$write=fwrite($fp,$out);
				fclose($fp);
				
				$sld_url_displayed_legend=V1_SLD_URL."/".$sldFileNameAfterLegendRewriting;
				...
				$json.=",\"legend\": \"".$sld_url_displayed_legend."\"";
				
				$json.=",\"geoserver\": \"".URL_GEOSERVER."\"";
				//if($p_occ_data)
				//{				
					$json.=",\"points_sld\": \"".URL_SITE."/synthesys/www/v1.4/sld/point_".$random.".sld\"";
					if($flagReprojection===TRUE&&strtolower($p_img)=='false'&&strlen($bboxReprojection)>0)
					{
						if(strlen($p_dest_epsg_projection)>0)
						{
							$json.=",\"dest_projection\": \"".$p_dest_epsg_projection."\"";
							
							$json.=",\"bbox_dest_projection\": \"".$bboxReprojection."\"";
				...
				$json.="]";	
					...
					$random3="/var/www/synthesys/www/v1.4/img/".md5($p_uri )."bckgrd_layers.png";
					$random4="/var/www/synthesys/www/v1.4/img/".md5($p_uri )."wms_layers.png";
							...
							for ($i=0;$i<count($arrayBackgroundStyle);$i++)
							{
								...
								else
								{
									...
									elseif($arrayBackgroundStyle[$i]['type']=="user_sld")
									{
										...
										//print($returnedXML);
										$path_towrite="/var/www/synthesys/www/v1.4/sld/"."_backsld_".$displayedLayer_sld.$random;
										/*print("returnedXML=");
										print($returnedXML);
										print("=");*/
...
*/
	...
	*/
	...
	{	//print("point=true");
		...
		$sld_url.=URL_GEOSERVER."?SERVICE=WMS&VERSION=1.1.1&TRANSPARENT=true&STYLES=&REQUEST=GetMap&SERVICE=WMS&EPSG=4326&layers=rest_points&EPSG=4326&format=image/png&bbox=".$bbox."&WIDTH=".$width."&HEIGHT=".$height."&SLD=".URL_SITE."/synthesys/www/v1.4/sld/point_".$random.".sld";
...
//print("----");
	...
	
	{
		...
		$legend_url=URL_GEOSERVER."/wms?REQUEST=GetLegendGraphic&VERSION=1.1.1&format=image/png&WIDTH=64&HEIGHT=36&";
					...
					$legend_url.="layer=topp:tdwg_level_3&LEGEND_OPTIONS=forceLabels:on;fontStyle:italic;fontSize:12&SLD=".$sld_url_displayed_legend;
	$com="convert  '$legend_url' '$random2_legend'";
	shell_exec($com);
		if($flagDisplayPoints===true)
		{
			$legend_url_point.=URL_GEOSERVER."/wms?REQUEST=GetLegendGraphic&LEGEND_OPTIONS=forceLabels:on;fontStyle:italic;fontSize:12&VERSION=1.1.1&format=image/png&WIDTH=20&HEIGHT=30&layer=rest_points&SLD=".URL_SITE."/synthesys/www/v1.4/sld/point_".$random.".sld";
...
//print($mlp);
	...
	//{
		...
		{
			if($p_no_header_for_tests===FALSE&&$flagGoodHeaders==TRUE)
			{
	...
	/*while($entry = $d->read()) { 
	 ...
	 if ($entry!= "." && $entry!= "..") { 
			...
			//modified ftheeten 20140910
			clearstatcache();
		     $f_last_modified = filemtime($sld_dir."/".$entry);
	
	
	
	if ($time-$f_last_modified >86400)
