...
<xsl:param name="position" select="order"/>
	...
	<xsl:choose>
		...
		<xsl:call-template name="symbology">
				<xsl:with-param name="color" select="'#E3FFDB'"/>
				<!-- el 5 de utm_regs2-->
		</xsl:call-template>
		</xsl:when>
	<xsl:when test="$position = 1">
		<xsl:call-template name="symbology">
				<xsl:with-param name="color" select="'#579343'"/>
		</xsl:call-template>
	</xsl:when>
	...
	<xsl:when test="$position = 2">
		...
		<xsl:when test="$position = 3">
			<xsl:call-template name="symbology">
					<xsl:with-param name="color" select="'#5787E7'"/>
					<!-- el 5 de utm_regs2-->
			</xsl:call-template>
			</xsl:when>
			<xsl:when test="$position = 4">
				<xsl:call-template name="symbology">
						<xsl:with-param name="color" select="'#000099'"/>
						<!-- el 5 de utm_regs2-->
				</xsl:call-template>
			</xsl:when>
	<xsl:when test="$position = 5">
		<xsl:call-template name="symbology">
				<xsl:with-param name="color" select="'#a7aaaf'"/>
		</xsl:call-template>
	</xsl:when>
	</xsl:choose>
</xsl:template>
