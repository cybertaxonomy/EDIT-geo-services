...
function ws_identify(data) {
	...
	function curves(data,columns,layer)//,layer)
			{
...
//console.log(item);
//console.warn(item.row.data);
              	if ( i !== (data.polygon.length -1))
									{
						to_print+="{\""+item.row.label+"\":{label:\""+item.row.label+"\", data:[["+i+","+item.row.data+"]]}},";
//console.warn(data.polygon.length);
									}				
										else {
					to_print+="{\""+item.row.label+"\":{label:\""+item.row.label+"\", data:[["+i+","+item.row.data+"]]}},";
								    	}
...
//			console.log("color is "+key);  
		...
		if (columns=='false3')
				{
			var options = {
		       // legend: { show: true, noColumns:2, container: $("#overviewLegend") },
		 legend: { show: false },
		        yaxis: { ticks: 10 },
	        	xaxis: { ticks: 0 },
		        bars: { show: true },			
...
var plot = $.plot(placeholder, data, options);
		...
		{
		        legend: { show: true, noColumns:5,container: $("#overviewLegend") },
