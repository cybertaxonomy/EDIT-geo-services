...
{
     if(isset($val ['geodata']['lat'])===true)
     {   
	foreach ($val ['geodata']['lat'] as $k=>$v)//($geodata[$key]['geodata']['lat'] as $k=>$v)
	{
...
$edit_url=$URL_WMS."?SERVICE=WMS&VERSION=1.1.1&TRANSPARENT=true&REQUEST=GetMap&SERVICE=WMS&EPSG=4326&layers=";
	...
	//added ftheeten 23/03/2009
				if ($_GET['raster']=='mercantour')
				{
					$edit_url.='topp:atbi_mercantour_raster,';
...
//commented ftheeten 23/03/2010

$edit_url.=",topp:rest_points&EPSG=4326&format=image/png&bbox=".$bbox."&WIDTH=".$width."&HEIGHT=".$height."&SLD=".$URL_SLD."/".$file_to_write;
//$edit_url.=",topp:rest_points&EPSG=4326&format=image/png&bbox=".$bbox."&WIDTH=".$width."&HEIGHT=".$height;


//print("SLD=");
//print($URL_SLD."/".$file_to_write);
//echo $edit_url;
//$image=imagecreatefrompng($edit_url);
//print $edit_url;
$r= "/var/www/edit/edit_wp5/v1/".(rand()%300).".png";
$c="convert '$edit_url' '$r'";
//$c="convert -transparent white '$edit_url' -transparent white '$r'";
//print($c);
shell_exec($c) ;
...
$c2="convert +append  'image.png'  '$legend_url' '$r'";
if ($legend=='1')
{
//	echo $c2;
shell_exec($c2);	
...
//$result= "/var/www/edit/edit_wp5/v1/".(rand()%300)."_result.png";
//$c="composite -transparent \"rgb(255,255,255)\"  $raster $r res:$result";
//$c="composite  $r $raster $result";
//$c="composite -gravity center '$raster' '$r': '$result'";
//shell_exec($c);
