...
$sld_path='/var/www/synthesys/www/geo/sld/temp/'.$user.'/analysis/';
$regs_path='/var/www/synthesys/www/geo/sld/temp/'.$user.'/analysis/'.$sld_num_regs;
$taxa_xml_path='/var/www/synthesys/www/geo/sld/temp/'.$user.'/analysis/'.$sld_taxa_rec;
$gen_path='/var/www/synthesys/www/geo/sld/temp/'.$user.'/analysis/'.$sld_genera;
...
$poligon_sql.="insert into point_pol (userid,code,the_geom,numreg,numtax) select p.userid, poly.code,poly.the_geom, COUNT(p.pk),count(distinct(p.genus)) from test_csvimportgispoints2 p INNER JOIN $polygon poly ON p.userid='$user' and poly.the_geom && p.the_geom and CONTAINS(poly.the_geom,p.the_geom) group by p.userid,poly.code,poly.the_geom";
//echo $poligon_sql;
