 ...
 // console.log("false");
	...
	//$("p[id='symbolize']").parent().children().attr('grup')
				...
				//console.log(p_color);
					...
					sld='http://taxonomicindex.br.fgov.be/edit_wp5/geo/layers_sld/'+l_name+'.php?params='+up+'/'+l_name+'/'+s_color+'/'+s_width+'/'+s_style;
				   if (typeof(eval('edit_'+s_layer))=='object')  //edit_group still not defined object (first time symbolization)
				   {
				//alert(layer_name);
				//layer_name"=new OpenLayers.Layer.WMS.Untiled( l_name,'http://193.190.116.6:8080/geoserver/wms',{layers:''+l_name,sld:sld,transparent:'true'});
				//eval(layer_name+"=new OpenLayers.Layer.WMS.Untiled( "+l_name+",'http://193.190.116.6:8080/geoserver/wms',{layers:''"+l_name+"',sld:'"+sld+"',transparent:'true'});
				...
				map.addLayer(layer_name);
				 ...
				 msie=($.browser.msie==true)?true:false; 
					if (msie)
					{
						...
						$('input.jqmdX').css('background','url(http://taxonomicindex.br.fgov.be/edit_wp5/edit_geo/prototype/JQ_win_files/close.gif)  no-repeat top left').css('width','15px');
						}else 
						{
							$('input.jqmdX').css('background','url(http://taxonomicindex.br.fgov.be/edit_wp5/edit_geo/prototype/JQ_win_files/close.gif) no-repeat top left');
						}
...
$("#images").css({'left':'950'});
	 ...
	 var height=$("#map").height();	 
			...
			$.get('http://taxonomicindex.br.fgov.be/edit_wp5/geo/test_scalebar.php',{bbox:bbox,w:width,h:height,units:units,label_size:label_size,l_size:l_size,intervals:intervals},function(s)

		        {

		        d='http://taxonomicindex.br.fgov.be/edit_wp5/geo/images/scalebars/'+s;
...
//				$("#tif_form,#bits_form").slideUp()
	...
	var img=$("#choose_windrose_form option:selected").val();
		var windrose_path="http://taxonomicindex.br.fgov.be/edit_wp5/geo/images/windroses/"+img;
