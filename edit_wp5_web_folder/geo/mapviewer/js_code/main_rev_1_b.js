...
*/
							...
							}) //polygon SLD click function

$('input.jqmdX').hover(function()
{
	$(this).addClass('jqmdXFocus')},function()
	{  
		$(this).removeClass('jqmdXFocus')}).focus(function()
		{
...
//issue 2 semi colons missing
		$("div#layer_symbol") .jqDrag('.jqDrag').jqResize('.jqResize') ;
		$("#picker").parent().attr('id',s_layer); //added semi colon
		$('div#layer_symbol').jqm({    overlay: 30 }); //added semi colon
		var f = $.farbtastic('#picker'); //the Farbtastic object is returned instead of the jQuery object. This allows you to use the Farbtastic methods and properties below.
		...
		.each(function () 
		{ 
			f.linkTo(this); //associa el valor seleccionat a this (a on i hagi class=colorwell)
			$(this).css('opacity', 0.75); 
		})
		.focus(function() 
		{ //quan fem focus sobre .colorwell
			if (selected) 
			{
...
{   //click on 'hide/show layers'
	...
	if (nexts.length >0)
	{
		...
		if (input.checked==true)
		{
			...
			$(nexts).css('visibility','visible'); //added semicolon

			$(ul).css('height',size); //added semicolon
		}
		else
		{
			$(nexts).css('visibility','hidden'); //added semicolon
			$(ul).css('height','20');
		}
	}
});
...
{   //click on 'hide/show layers'
	...
	if ($(label).hasClass('label'))  //by default hasClas 'label'
	{
		...
		$(input).trigger('click');
		$(input).attr('checked',true); //added semicolon
		//$(input).trigger('click');
		//$(input).trigger('click');
...
$("div[id='show_WMS']").click(function()
{
	if ($(this).hasClass('show_it'))  //it's hidden
	{
		$(this).children().eq(0).text('Hide remote WMS description'); //added semicolon
		$(this).removeClass('show_it');
		$("div[id='message']").fadeIn('slow'); //added semicolon
	}
	else 
	{                    //is visible
		$(this).children().eq(0).text('Show remote WMS description'); //added semicolon
		$(this).addClass('show_it');
		$("div[id='message']").fadeOut('slow'); //added semicolon
	}
}); //added semicolon

$("#wms_checkbox ul").css('height','auto'); //added semicolon
	...
	$.post('http://edit.br.fgov.be/edit_wp5/geo/remote_capabilities/capabilities.php', {url:remote_url},function(d)
	{
		$.ajax(
		{
			...
			error: function() 
			{
				...
				{
					$("#wms_form").empty(); //added semicolon
				}
				if ($("#abstract").size()>0)
				{
					$("#abstract").empty(); //added semicolon
				}
			},
