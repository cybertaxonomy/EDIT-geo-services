...
//	alert($("iframe#info").attr('src'));
 ...
 if(submit_button.hasClass("add"))(submit_button.removeClass("add").val('Symbolyze it'));});
  //submit is the id of
//    $("#color_form11 form ,#symbol_form1 form,#size_form1 form").empty(); 
...
var iframe=$("iframe#info");    
    var submit=iframe.contents().find('#genusSelect option:selected');
        ...
        //        var selected_genus2=iframe.contents().find('#categorySelect option:selected').val();
                genus_val.push(parseInt(this.value)); //this.value donarÃ  0,1,2...o Bubas,Caccobius...????
                genus_name.push(this.text); //this.value donarÃ 
				d.push(this.text);
                // si 2Âª... submit.each( function(index,data) {selected_genus.push(index)}; -->NO NECESSARI!
    ...
    //        $("#ex2").animate({width:'390',height:'235'},"slow");
	 ...
	 $('#info').fadeOut("slow");$("#info").hide(); //si no afegim hide, queda una part de l'iframe (IE, Ã©s clar!!)
 $("#send1").show();
