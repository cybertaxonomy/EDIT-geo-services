...
eval("edit_"+data.id+".setVisibility(false)");
	...
	ajax_show=true;					
	$.ajax({url:'http://edit.br.fgov.be/edit_wp5/geo/point_pol/ppol_adaptat.php?polygon='+polygon+'&user='+userid+'&sld='+user_info,type:'get',user:userid,polygon:polygon,dataType:'json',timeout:120000,
error: function() {alert("some error; possibly no selected polygon crosses any point")} ,success:function(j)
						{						
	bbox=j['bbox'];
					sld_num_regs="http://edit.br.fgov.be/edit_wp5/geo/sld/"+j['sld_num_regs'];	
...
//alert(sld_num_regs);												
					sld_num_genus="http://edit.br.fgov.be/edit_wp5/geo/sld/"+j['sld_genera'];	
					...
					sld_taxa_record="http://edit.br.fgov.be/edit_wp5/geo/sld/"+j['sld_taxa_rec'];							
...
var fill_it=function(layerName)
{
			switch (layerName) {
					...
					{	
						...
						$("img[id='num_regs']").attr('src',num_regs_legend);
							$("table[id='num_regs'] a:last").attr('href',"http://edit.br.fgov.be/edit_wp5/geo/curves/edit_user_curves.php?layer=numregs&user="+userid+"&bbox="+bbox+"&layer2=topp:"+polygon+"&sld="+sld_num_regs);
	...
	extra+='<br><a href="http://edit.br.fgov.be/edit_wp5/geo/curves/edit_user_curves.php?layer=numregs&user='+userid+'&bbox='+bbox+'&layer2=topp:'+polygon+'&sld='+sld_num_regs+'"  target="blank">Show Number of Records graphic on new window</a>';
					...
					if ($("#analysis_lay table img[id='num_genus']").length>0)
						{
							...
							$("table[id='num_genus'] a:last").attr('href',"http://edit.br.fgov.be/edit_wp5/geo/curves/edit_user_curves.php?layer=numregs&user="+userid+"&bbox="+bbox+"&layer2=topp:"+polygon+"&sld="+sld_num_genus);
...
extra='<table  id="num_genus"><tr><td><input id="num_genus"  grup="analysis" type="checkbox" visibility="visible" onclick="toogle_ppol(this);c_legend(this)"/><label style="font-size:11">Number of Genus</label></td>';
	...
	extra+='<br><a href="http://edit.br.fgov.be/edit_wp5/geo/curves/edit_user_curves.php?layer=numgenus&user='+userid+'&bbox='+bbox+'&layer2=topp:'+polygon+'&sld='+sld_num_genus+'"  target="blank">Show Number of Genus graphic on new window</a>';
		$("#analysis_lay").append(extra);
...
$("table[id='num_genus'] img").fadeIn("slow"); $(this).text('Hide legend');
						...
						if ($("#analysis_lay table img[id='taxa_rec']").length>0)
							{
								...
								$("table[id='taxa_rec'] a:last").attr('href',"http://edit.br.fgov.be/edit_wp5/geo/curves/edit_user_curves.php?layer=tax/reg&user="+userid+"&bbox="+bbox+"&layer2=topp:"+polygon+"&sld="+sld_taxa_record);
	...
	extra+='<br><a href="http://edit.br.fgov.be/edit_wp5/geo/curves/edit_user_curves.php?layer=tax/reg&user='+userid+'&bbox='+bbox+'&layer2=topp:'+polygon+'&sld='+sld_taxa_record+'"  target="blank">Show Number of Records graphic on new window</a>';
		$("#analysis_lay").append(extra);
