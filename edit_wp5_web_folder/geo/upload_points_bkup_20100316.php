...
//$sld_dir = DIR_PLATFORM."/edit_wp5/geo/sld"; 
//updated by ftheeten 26/02/2010 -> corrected by ftheeten 03/03/2010 ('$sld_file' shouldn't appear in the path)
//$sld_dir=DIR_PLATFORM."/synthesys/www/fitxers/sld_mapviewer/$sld_file";
//$sld_dir=DIR_PLATFORM."/synthesys/www/fitxers/sld_mapviewer";
$sld_dir=SLD_DIR;
...
//	 echo $filename;
	...
	$tf = $upload_dir.'/'.md5(rand()).".test";
        ...
        //move_uploaded_file($_FILES["file"]["tmp_name"],DIR_PLATFORM."/edit_wp5/geo/loaded_CSVs/" . $_FILES["file"]["name"]);
	move_uploaded_file($_FILES["file"]["tmp_name"],UPLOAD_DIR. $_FILES["file"]["name"]);

			$msg .= " File Name: " . $_FILES["file"]["tmp_name"]. ", ";
			$msg .= " File Size: " . @filesize($_FILES['file']['tmp_name']);

      //$msg .= "Stored in: " . DIR_PLATFORM."/edit_wp5/geo/loaded_CSVs/".$_FILES["file"]["name"];
      //$path=DIR_PLATFORM."/edit_wp5/geo/loaded_CSVs/".$_FILES["file"]["name"];
...
function third($userid,$sld_file)
{
	...
	$sld_path_towrite=SLD_DIR."/$sld_file";
...
switch ($s)
{
  case  3: third($userid,$sld_file);
		   $j.=",genus_sld: URL_SITE.'/edit_wp5/geo/sld/".$sld_file. "'}";
			break;
  case  4: fourth($userid,$sld_fourth_file);
		$j.=",fourth_sld: URL_SITE.'/edit_wp5/geo/sld/".$sld_fourth_file. "'}";

			break;

  case  5: third_fourth($userid,$sld_sp_file);
			$j.=",sp_sld: URL_SITE.'/edit_wp5/geo/sld/".$sld_sp_file. "'}";			
			break;
...
{
  case  3: third($userid,$sld_file);
		   $j.=",genus_sld: '".SLD_URL1.$sld_file. "'}";
			break;
  case  4: fourth($userid,$sld_fourth_file);
		$j.=",fourth_sld: '".SLD_URL1.$sld_fourth_file. "'}";

			break;

  case  5: third_fourth($userid,$sld_sp_file);
			$j.=",sp_sld: '".SLD_URL1.$sld_sp_file. "'}";			
			break;
  case  34: third($userid,$sld_file);fourth($userid,$sld_fourth_file);
			 $j.=",genus_sld: '".SLD_URL1.$sld_file. "'";
			$j.=",fourth_sld: '".SLD_URL1.$sld_fourth_file. "'}";
