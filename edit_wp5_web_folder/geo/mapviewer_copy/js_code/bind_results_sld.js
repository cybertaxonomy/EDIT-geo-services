...
if (msie)
{
 ...
 $('input.jqmdX').css('background','url(http://taxonomicindex.africamuseum.be/edit_wp5/geo/mapviewer/close.gif)  no-repeat top left').css('width','15px');
}
else 
{
$('input.jqmdX').css('background','url(http://taxonomicindex.africamuseum.be/edit_wp5/geo/mapviewer/close.gif) no-repeat top left');
}
...
$('#content').remove();
 $.get('http://taxonomicindex.africamuseum.be/edit_wp5/geo/queries/sp_query_sld.php', {param: bbox.join('&'),user:userid }, function(data) {$("#query_container").append(data); bindBehaviors(data);$("#repaginate2").show()});
