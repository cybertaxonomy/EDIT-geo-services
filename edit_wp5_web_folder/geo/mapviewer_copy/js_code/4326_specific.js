    ...
    serialized = new OpenLayers.Layer.WMS.Untiled( "Your symbolized polygons",
		        "http://193.190.223.53:8080/geoserver/wms", {layers:'topp:serialized_pols',GROUP:'remote',sld: 'http://taxonomicindex.africamuseum.be/edit_wp5/geo/permanent_sld/transparent_pols.sld',transparent:"true",format:t_img_format},{opacity:1},{'reproject': false});
		 ...
		 polygonLayer.features[id].attributes.strokeWidth=$("#stroke_width_form option:selected").val();
		  polygonLayer.features[id].attributes.strokeDashstyle=$("#stroke_style_form option:selected").val();
		   ...
		   var wkt= new OpenLayers.Format.WKT(map.baseLayer.projection);
			...
			$.ajax({url:'http://taxonomicindex.africamuseum.be/edit_wp5/geo/queries/data_insert.php',
							...
							//	alert("polygon inserted on dbase");
								//let's create SLD
		        $.ajax({url:'http://taxonomicindex.africamuseum.be/edit_wp5/geo/gml_sld.php',
...
if (msie)
{$('input.jqmdX').css('background','url(http://taxonomicindex.africamuseum.be/edit_wp5/geo/mapviewer/img/close.gif)  no-repeat top left').css('width','15px');}
else {$('input.jqmdX').css('background','url(http://taxonomicindex.africamuseum.be/edit_wp5/geo/mapviewer/img/close.gif) no-repeat top left');}
$('input.jqmdX').hover(function(){$(this).addClass('jqmdXFocus')},function(){        $(this).removeClass('jqmdXFocus')}).focus(function(){this.hideFocus=true;$(this).addClass('jqmdXFocus')}).blur(function(){$(this).removeClass('jqmdXFocus')}).click(function(){$('#ex2').hide();  });
