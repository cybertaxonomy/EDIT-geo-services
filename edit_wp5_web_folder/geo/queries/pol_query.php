	...
	//the_geom && GeomFromText('POINT($x $y)',4326)";
	 ...
	 else
	 {
      ...
      while ($row = pg_fetch_array($query_result, NULL, PGSQL_ASSOC)) 
				{   
			$sql2="select  COUNT(p.pk) as records,count(distinct(p.genus)) as genus from user_points p INNER JOIN $polygon poly ON p.userid='pou' and p.the_geom && poly.the_geom and poly.code='".$row['code']."' and CONTAINS(poly.the_geom,p.the_geom) group by p.userid,poly.code,poly.the_geom";
