...
//						   alert("this is mozilla!");
	   //console.log($.browser.version);
        //if ($.browser.product$ =="Firefox" && $.browser.version !== '1.8.1.14')
 		//{ 
//	     if(window.console) {
		$.browser.version !== '1.8.1.14' {
		alert ("If you are using Firebug over Firefox 3 you have to uninstall it. Sorry...");
       }
      }
    
	else { 
alert("This site still is not ready for your browser. You should use Firefox...");
// 	   if ($.browser.msie) { 
	}
...
//	edit_vectorial.push(edit_lay);																					edit_no_raster_styles.push(edit_style);
													  }
													}; // fi else de contains	
						   ...
						   switch (format)
						{
					      ...
					      case 'PDF/jpeg':
											{
											new_format="image/jpeg";					                        	$.get('http://edit.csic.es/fitxers/images/makepdf2.php', { edit_raster: edit_raster.join('/'),edit_vectorial:edit_vectorial,edit_sld:edit_sld,remote_layers:remote_layers,legend:legend,format: new_format,bbox: bbox},function(url_image){bindFrameActions();$("iframe#info").attr("src",url_image);$("#ex2").show();});  
											}
											break;
			...
			$.get('http://edit.csic.es/fitxers/images/makepdf2.php', { edit_raster: edit_raster.join('/'),edit_sld:edit_sld,edit_vectorial:edit_vectorial,legend:legend,format: new_format,bbox:bbox,remote_layers:remote_layers.join('|')},function(url_image){bindFrameActions();$("iframe#info").attr("src",url_image);$("#ex2").show();});  
							}
											break;
						case 'png/gray': 					
											{
...
edit_raster:edit_raster.join('/'),edit_sld:edit_sld,edit_vectorial:edit_vectorial,legend:legend,format:"png/gray",bbox:bbox,remote_layers:remote_layers.join('|')},function(url_image){bindFrameActions();$("iframe#info").attr("src",url_image);$("#ex2").show();});  
												}
																break;
							case 'jpeg/gray': 					
							{
																						new_format="image/jpeg";
																		$.get('http://edit.csic.es/fitxers/images/images.php', {											edit_raster:edit_raster.join('/'),edit_sld:edit_sld,edit_vectorial:edit_vectorial,legend:legend,format:"jpeg/gray",bbox:bbox,remote_layers:remote_layers.join('|')},function(url_image){bindFrameActions();$("iframe#info").attr("src",url_image);$("#ex2").show();});  
																							}
																											break;
																												  case 'gif/gray': 					
																											{
																																										new_format="image/gif";
																																						$.get('http://edit.csic.es/fitxers/images/images.php', {											edit_raster:edit_raster.join('/'),edit_sld:edit_sld,edit_vectorial:edit_vectorial,legend:legend,format:"gif/gray",bbox:bbox,remote_layers:remote_layers.join('|')},function(url_image){bindFrameActions();$("iframe#info").attr("src",url_image);$("#ex2").show();});  
																																											}
																																															break;
						...
						$.get('http://edit.csic.es/fitxers/images/images.php', { edit_raster: edit_raster.join('/'),edit_sld:edit_sld,edit_vectorial:edit_vectorial,legend:legend,format: new_format,bbox:bbox,remote_layers:remote_layers.join('|')},function(url_image){bindFrameActions();$("iframe#info").attr("src",url_image);$("#ex2").show();});  
					}
/*						if (edit_raster=='')
                        { 
						if (remote_layers=='')
						{	
						 ...
						 $.get('http://edit.csic.es/fitxers/images/images_creation_sld2.php', { edit_layers: edit_layers.join('/'),edit_sld:edit_sld,format: new_format ,bbox: bbox},function(url_image){bindFrameActions();$("iframe#info").attr("src",url_image);$("#ex2").show();});  
					   }
					else { //with remote_layers (non EDIT)
						console.info("sí remote no raster layers  "+edit_raster);
$.get('http://edit.csic.es/fitxers/images/images_creation_sld2.php', { edit_layers: edit_layers.join('/'),edit_sld:edit_sld, format: new_format ,bbox: bbox,remote_layers:remote_layers.join('|')},function(url_image){ $("iframe#info").attr("src",url_image);$("#ex2").show();});						
					     }
				      } // fi IF raster
				      else //with edit Raster Layers
				    {
				    if  (remote_layers=='')
				    {
					$.get('http://edit.csic.es/fitxers/images/images_creation_sld.php', { edit_layers: edit_layers.join('/'),edit_sld:edit_sld, format: new_format ,bbox: bbox},function(url_image){ $("iframe#info").attr("src",url_image);$("#ex2").show();});	
				    }
				   else
				   {								$.get('http://edit.csic.es/fitxers/images/images_creation_sld.php', { edit_layers: edit_layers.join('/'),edit_sld:edit_sld, format: new_format ,bbox: bbox,remote_layers:remote_layers.join('|')},function(url_image){ $("iframe#info").attr("src",url_image);$("#ex2").show();});		
				}				        
				}*/
					  break;
						  } //FI switch 
