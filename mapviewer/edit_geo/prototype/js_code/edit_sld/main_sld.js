$(document).ready(function(){
		if ($.browser.mozilla)
	    {
	    	if	($.browser.version !== '1.8.1.15' || $.browser.version !== '2.0.0.15'  || $.browser.version !== '1.8.1.15') {
	//   console.log ($.browser.version);
	//ƒ+		alert ("If you are using Firebug on Firefox 3 you have to uninstall it. Sorry...")
	  //     } //else {};
	      }
	    }
...
var path="http://edit.csic.es/fitxers/XMLs/"+context;
mainMap=path;

		}
   getContext();
var extent=cadena2;
/*$('#query_AOI').click(function(){$.getScript('js_code/bind_it_sld.js');$('#content').remove(); */
/*
var aoiBox=config.objects.mainMap.getParam("aoi");var x_ul=aoiBox[0][0];var y_ul=aoiBox[0][1];var x_br=aoiBox[1][0];var y_br=aoiBox[1][1];bbox=[];bbox.push(x_ul,y_ul,x_br,y_br)});$(".tabs").hide();$('#workspace').hide();
*/
...
$('#query_result,#ex2,#ex3,#ex_info,.tabs,div[@id*="_lay"],#help,dd:not(:first),#workspace,#user_analysis_legend').hide();

$("dt").click(function(){
	//$("dd:visible").slideUp("slow");$(this).parent().next().slideDown("slow");return false});
//CHANGES
   ...
   if(selected) {
        ...
        $("#ex2,#color_form").fadeIn();
              ...
              //PRIMER ÚS DE L'EINA. CARREGUEM SCRIPT.
				$("#id").addClass("selected");
			       $.getScript('js_code/edit_sld/symbology_sld.js');      
			      
				$("iframe#info").fadeIn();
				};
   });	
//FIRING WMS SEARCH IN WMS-SITES.COM
	...
	$.getJSON('http://edit.csic.es/edit_geo/prototype/json/edit3.json', function(data)
	 {
  ...
  $('#iberian_context').click(function() {	
    ...
    // ?????
	$('#a[id="tab1"],a[id="tab2"],a[id="tab3"]').show();
    var extent='iberia';
	//console.log(extent);
	getlayers(extent);
	});
...
//LAYERS, METADATA AND LEGEND CONTROL
...
//SHOW POINTS LEGEND WHEN CLICK
$("#puntos_info,#user_dates").click(function(){$("#puntos_lay").slideDown("slow");});
//ALLOW PRINTING
$("#download_data").one('click',function()
        {
     $.getScript('js_code/edit_sld/print_sld.js');

        } );
//FIRST CLICK--->WE GET POINTS LEGEND
...
//jqDnr.js
	...
	else if(typeof fb.callback=='function'){fb.callback.call(fb,fb.color)};};fb.absolutePosition=function(el){var r={x:el.offsetLeft,y:el.offsetTop};if(el.offsetParent){var tmp=fb.absolutePosition(el.offsetParent);r.x+=tmp.x;r.y+=tmp.y};return r};fb.pack=function(rgb){var r=Math.round(rgb[0]*255);var g=Math.round(rgb[1]*255);var b=Math.round(rgb[2]*255);return'#'+(r<16?'0':'')+r.toString(16)+(g<16?'0':'')+g.toString(16)+(b<16?'0':'')+b.toString(16)};fb.unpack=function(color){if(color.length==7){return[parseInt('0x'+color.substring(1,3))/255,parseInt('0x'+color.substring(3,5))/255,parseInt('0x'+color.substring(5,7))/255]}else if(color.length==4){return[parseInt('0x'+color.substring(1,2))/15,parseInt('0x'+color.substring(2,3))/15,parseInt('0x'+color.substring(3,4))/15]}};
	fb.HSLToRGB=function(hsl){var m1,m2,r,g,b;var h=hsl[0],s=hsl[1],l=hsl[2];m2=(l<=0.5)?l*(s+1):l+s-l*s;m1=l*2-m2;return[this.hueToRGB(m1,m2,h+0.33333),this.hueToRGB(m1,m2,h),this.hueToRGB(m1,m2,h-0.33333)]};fb.hueToRGB=function(m1,m2,h){h=(h<0)?h+1:((h>1)?h-1:h);if(h*6<1)return m1+(m2-m1)*h*6;if(h*2<1)return m2;if(h*3<2)return m1+(m2-m1)*(0.66666-h)*6;return m1};fb.RGBToHSL=function(rgb){var min,max,delta,h,s,l;
		var r=rgb[0],g=rgb[1],b=rgb[2];min=Math.min(r,Math.min(g,b));max=Math.max(r,Math.max(g,b));delta=max-min;l=(min+max)/2;s=0;if(l>0&&l<1){s=delta/(l<0.5?(2*l):(2-2*l))};h=0;if(delta>0){if(max==r&&max!=g)h+=(g-b)/delta;if(max==g&&max!=b)h+=(2+(b-r)/delta);if(max==b&&max!=r)h+=(4+(r-g)/delta);h/=6};return[h,s,l]};$('*',e).mousedown(fb.mousedown);fb.setColor('#000000');
