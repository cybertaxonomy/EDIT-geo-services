...
*/
	...
	$('#id').click(function()
   {   
   $.getScript('js_code/new_symbology.js');
   //$("#ex2").show();
   });	
   ...
   //alert("llegenda hauria de no ser modificada");
	...
	$("#format_form,#download_data").show();
		   $("#download_data a").one('click',function()
	        {
	...
	getlayers(extent);
		...
		$('ul.Layers a').click(function() {		
			...
			$.getJSON('json/edit3.json',  function(data) {	
				...
				$('a[@href=#analysis_metadata]').addClass('selected');//.prev().removeClass('selected');				
				}
				if (entry.theme=='enviranalysis') {	
				    html='<h5>'+info['Abstract']+'</h5>';	
					html+='<a href='+info['link']+'>Add this layer<br>';						
				...
				$('a[@href=#enviranalysis_metadata]').addClass('selected');//.prev().removeClass('selected');				
				}
				if (entry.theme=='landcover') {					
				   html='<h5>'+info['Abstract']+'</h5>';
				   html+='<a href='+info['link']+'>Add this layer<br>';
