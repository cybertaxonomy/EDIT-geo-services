...
$('table.paginated').each(function(){var currentPage=0;var numPerPage=10;
var $table=$(this);
...
if (msie)
{
      $('input.jqmdX').css('background','url(http://edit.csic.es/edit_geo/prototype/JQ_win_files/close.gif) no-repeat').css('width','18px');
}
else {
$('input.jqmdX').css('background','url(http://edit.csic.es/edit_geo/prototype/JQ_win_files/close.gif) no-repeat top left;');
 }

$.get('http://edit.csic.es/fitxers/queries/sp_query_aoi.php', {param: bbox.join('&') }, function(data) {$('#content').empty();$("#query_container").append(data);bindBehaviors(data)});
